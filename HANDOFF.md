...
5. **LLM : le client choisit (décision Tym, 15 août)**. Tym ne paie pas Claude pour tout le monde. Modèle : (a) **IA locale par défaut**, souveraine, incluse ; (b) **Claude en option, avec la clé du client** (`store.config.llm.claudeApiKey`, chiffrée) : sa clé, sa facture, notre honnêteté ("hébergé en France par défaut ; si vous voulez le meilleur modèle, c'est votre clé"). À faire : (1) clé LLM par store + sélection dans Réglages ; (2) **passer le local de qwen2.5:3b à un modèle plus gros** (7b si la RAM du VPS suit) : le 3b ignore les consignes (recommande un épuisé, hallucine des produits hors catalogue), le code déterministe posé le 15/08 sert de filet mais ne remplace pas un modèle correct. Puis plan coûts : cache sémantique + prompt caching côté Claude.
