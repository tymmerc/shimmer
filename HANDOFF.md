...
- **UX vendeur : dock discret ancré (16 sept, décision Tym)** : le SDK n'intercepte PLUS le focus. Le visiteur tape dans la barre native du thème (suggestions natives comprises), Shimmer s'ouvre à Entrée/submit (≥2 car.). Fini l'overlay plein écran + voile sombre : un DOCK discret ancré sous la barre (`position`/`getBoundingClientRect`, repositionné au scroll/resize, pleine largeur <560px), produits en HAUT, petite question du vendeur en BAS, pied avec « Voir les résultats classiques → » (rejoue la recherche native via `form.submit()` + flag `bypassNext`) et « Fermer ». La barre native DEVIENT le champ de suite de conversation (placeholder « Précisez… », restauré à la fermeture). Gras markdown rendu (esc + `<strong>`). Clic dehors/Escape ferme. Même déclencheur que l'enrôlement témoin → groupes comparables. Reste du pack "intégration impeccable" : Shadow DOM, auto-habillage aux couleurs de la boutique, résultats produits instantanés pendant la frappe, banc d'essai thèmes Shopify réels.
