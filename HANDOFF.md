...
- **PIÈGE nginx dev + SDK** : dans `12-dev-https.conf`, le bloc `location ^~ /shimmer/` SAUTE l'évaluation des locations regex → une location regex pour le SDK y est du code mort et l'URL du SDK renvoyait l'index.html du site (vécu le 31/08, corrigé en `location ^~ /shimmer/sdk/`). Toujours vérifier le CONTENU servi, pas le code HTTP.
...
- **Consentement RGPD dans le SDK (31 août)** : `data-consent="auto"` (défaut) détecte la CMP (TCF v2, Cookiebot, Axeptio, tarteaucitron) ; tant que pas de signal → MODE SESSION (vendeur fonctionnel, id en mémoire `svid_`, AUCUN cookie, pas d'expérience holdout, pas d'attribution) ; accord → cookie 365 j + boot mesuré ; refus → cookie effacé, mesure coupée ; sans CMP détectée sous 3 s → plein mode (posture du site). Modes `strict` / `granted` / `denied` + API `Shimmer.consent(bool)` + event DOM `shimmer:consent`. Les refus sortent des DEUX groupes → zéro biais, preuve juste plus lente. Vérifié 4/4 sur dev (`/shimmer/consent-test.html`).
- **Plafond de dépense IA par boutique (15 sept)** : `packages/core/src/llm-budget.ts`. Compteur mensuel Redis (`llmspend:{storeId}:{YYYY-MM}`, micro-euros entiers, TTL 45 j), plafond défaut **10 €/mois** surchargeable via `store.config.llm.budgetEURMonthly` (0 = premium coupé). Garde dans `ClaudeClient.complete()` : tout appel PAYANT passe `options.storeId` (vendeur + SAV branchés) ; au plafond → bascule silencieuse sur l'IA locale (jamais de vendeur mort), pas de sauvetage Claude non plus ; chaque appel payé est compté (tokens réels × barème €/Mtoken, modèle inconnu = tarif conservateur). Fail-open si Redis/DB toussent. Exposé dans `/api/integration/status` (`llm.budget`) + ligne dans l'admin Intégration (rouge si plafond atteint). Limite v1 : `stream()` non compté (le vendeur utilise complete), cross-sell non branché (génération marchande ponctuelle).
- **UX vendeur : dock discret ancré (16 sept, décision Tym)** : le SDK n'intercepte PLUS le focus. Le visiteur tape dans la barre native du thème (suggestions natives comprises), Shimmer s'ouvre à Entrée/submit (≥2 car.). Fini l'overlay plein écran + voile sombre : un DOCK discret ancré sous la barre (`position`/`getBoundingClientRect`, repositionné au scroll/resize, pleine largeur <560px), produits en HAUT, petite question du vendeur en BAS, pied avec « Voir les résultats classiques → » (rejoue la recherche native via `form.submit()` + flag `bypassNext`) et « Fermer ». La barre native DEVIENT le champ de suite de conversation (placeholder « Précisez… », restauré à la fermeture). Gras markdown rendu (esc + `<strong>`). Clic dehors/Escape ferme. Même déclencheur que l'enrôlement témoin → groupes comparables. **Shadow DOM (17 sept)** : dock + chatbot montés dans `#shimmer-root` (shadow ouvert, `:host { all: initial }`), styles injectés dedans ET en light DOM (cross-sell vit dans la page marchand). Clic dehors via `composedPath()`. Prouvé sur `dev.tymmerc.eu/shimmer/hostile-theme.html` (Comic Sans rouge, `button { all: unset }`, bordures partout) : le dock reste intact. Pour inspecter en console : `document.getElementById('shimmer-root').shadowRoot.querySelector('.shimmer-dock')`. Reste du pack "intégration impeccable" : auto-habillage aux couleurs de la boutique, résultats produits instantanés pendant la frappe, banc d'essai thèmes Shopify réels.
- Tests API : 197/197 verts (13 fichiers).
...
5. **LLM : le client choisit (décision Tym, 15 août)**. Tym ne paie pas Claude pour tout le monde. Modèle : (a) **IA locale par défaut**, souveraine, incluse ; (b) **Claude en option, avec la clé du client** (`store.config.llm.claudeApiKey`, chiffrée) : sa clé, sa facture, notre honnêteté ("hébergé en France par défaut ; si vous voulez le meilleur modèle, c'est votre clé"). À faire : (1) clé LLM par store + sélection dans Réglages ; (2) **modèle local = qwen2.5:3b, décision du 15/08 après benchmark** : le 7b (déjà téléchargé) est ~35 s/question sur ce CPU 8 cœurs sans GPU (vs 9-17 s en 3b), inacceptable en barre de recherche ; la justesse sur le stock vient du code (garde-fous déterministes : épuisé jamais recommandé, produit nommé toujours vu, catégorie générique → en stock, redirection même catégorie), pas du modèle. Testé 8/8 questions stock justes en 3b. PIÈGE : ne jamais laisser deux modèles chargés (keep_alive) → CPU saturé, latence ×2. Ollama : timeout 120 s, keep_alive 30 m. Si un client veut mieux : Claude avec sa clé. Puis plan coûts : cache sémantique + prompt caching côté Claude.
