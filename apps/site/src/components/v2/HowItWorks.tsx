...
export function HowItWorks() {
  return (
    <section id="process" className="relative z-10 w-full scroll-mt-16 px-6 py-28 md:px-12 md:py-40">
      <div className="mx-auto max-w-[1400px]">
        ...
        <h2 className="mb-16 max-w-[22ch] font-display text-[clamp(34px,5vw,72px)] font-normal leading-[0.95] tracking-tightest text-paper">
        ...
        <div className="grid grid-cols-1 gap-x-10 gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((s, i) => (
            ...
            >
              ...
              <h3 className="mt-4 font-display text-2xl leading-tight text-paper md:text-[28px]">{s.title}</h3>
              <p className="mt-4 text-pretty text-base leading-relaxed text-paper/65 md:text-lg">{s.text}</p>
            </motion.div>
          ))}
        </div>
      </div>
