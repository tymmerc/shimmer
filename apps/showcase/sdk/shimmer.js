...
`);p=g.pop()||"";let u="";for(let v of g)if(v.startsWith("event: "))u=v.slice(7).trim();else if(v.startsWith("data: ")){let H=v.slice(6);try{let b=JSON.parse(H);u==="metadata"?t.onMeta(b):u==="token"?t.onToken(b.text):u==="done"?t.onDone(b.fullText):u==="error"&&t.onError?.(b.error)}catch{}}}}).catch(d=>{d.name!=="AbortError"&&t.onError?.(d.message)}),r}chatMessage(e,t){return this.request("POST","/api/chat/message",{message:e,sessionToken:t,stream:!1})}reviewStats(e){let t=e?`?productId=${e}`:"";return this.request("GET",`/api/reviews/stats${t}`)}productReviews(e,t=1){return this.request("GET",`/api/reviews/product/${e}?page=${t}&limit=10`)}crossSell(e,t=4){return this.request("GET",`/api/catalog/cross-sell/product/${e}?limit=${t}`)}crossSellEvents(e){let t=JSON.stringify({events:e}),s=`${this.apiUrl}/api/catalog/cross-sell/events`;if(typeof navigator<"u"&&navigator.sendBeacon){let i=new Blob([t],{type:"application/json"});try{if(navigator.sendBeacon(s,i))return Promise.resolve()}catch{}}return fetch(s,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.apiKey}`},body:t,keepalive:!0}).then(()=>{}).catch(()=>{})}};function U(n){if(document.getElementById("shimmer-sdk-styles"))return;let e=document.createElement("style");e.id="shimmer-sdk-styles",e.textContent=`
  ...
  `,document.head.appendChild(e)}var T=class{constructor(e,t,s){this.client=e;this.labels=t;this.searchSelector=s;this.debounceTimer=null;this.sessionToken=null;this.createOverlay(),this.hookExistingInputs()}createOverlay(){this.overlay=document.createElement("div"),this.overlay.className="shimmer-widget shimmer-search-overlay",this.overlay.innerHTML=`
    ...
    `;let o=Array.from(e.querySelectorAll(".sx-card"));if(o.forEach((r,l)=>{let d=Number(r.dataset.productId),h=t.items.find(m=>m.product.id===d);h&&r.addEventListener("click",m=>{m.target.closest(".sx-add")||(this.trackEvent({product_id:t.reference.id,target_id:h.product.id,role:h.role,event_type:"click",position:l}),L({ref_id:t.reference.id,target_id:h.product.id,role:h.role,position:l,ts:Date.now()}),this.flushEvents(),this.opts.onProductClick(h))})}),e.querySelectorAll(".sx-add").forEach(r=>{let l=Number(r.dataset.add),d=t.items.find(m=>m.product.id===l);if(!d)return;let h=o.findIndex(m=>m.dataset.productId===String(l));r.addEventListener("click",m=>{m.preventDefault(),m.stopPropagation(),this.trackEvent({product_id:t.reference.id,target_id:d.product.id,role:d.role,event_type:"add",position:h}),L({ref_id:t.reference.id,target_id:d.product.id,role:d.role,position:h,ts:Date.now()}),this.flushEvents(),this.opts.onProductClick(d)})}),typeof IntersectionObserver<"u"){let r=new WeakSet,l=new Map,d=new IntersectionObserver(h=>{for(let m of h){if(r.has(m.target))continue;let p=m.target;if(m.isIntersecting&&m.intersectionRatio>=.5){if(l.has(p))continue;let f=window.setTimeout(()=>{r.add(p),l.delete(p),d.unobserve(p);let y=Number(p.dataset.productId),g=t.items.find(v=>v.product.id===y);if(!g)return;let u=o.indexOf(p);this.trackEvent({product_id:t.reference.id,target_id:g.product.id,role:g.role,event_type:"impression",position:u})},300);l.set(p,f)}else{let f=l.get(p);f!==void 0&&(window.clearTimeout(f),l.delete(p))}}},{threshold:[0,.5,1]});o.forEach(h=>d.observe(h))}if(typeof window<"u"){let r=()=>this.flushEvents();window.addEventListener("pagehide",r,{once:!1}),window.addEventListener("beforeunload",r,{once:!1})}}},a=class a{constructor(e){this.searchWidget=null;this.chatWidget=null;this.assistHistory=[];this.assistKnownCriteria={};this.config=e,this.theme={...B,...e.theme},this.labels=D[e.locale||"fr"],this.client=new E(e.apiUrl,e.apiKey)}static init(e){a.instance&&a.instance.destroy();let t=new a(e);return U(t.theme),t.searchWidget=new T(t.client,t.labels,e.searchSelector),t.chatWidget=new k(t.client,t.labels),a.instance=t,t}static get assistant(){return{chat:a.chat,reset:a.resetChat}}static search(e){if(!a.instance)throw new Error("Shimmer not initialized. Call Shimmer.init() first.");return a.instance.client.search(e)}static openSearch(e){a.instance?.searchWidget?.open(e)}static async chat(e){if(!a.instance)throw new Error("Shimmer not initialized. Call Shimmer.init() first.");let t=a.instance;t.assistHistory.push({role:"user",content:e});let s=await t.client.assist(e,t.assistSessionToken,t.assistHistory.slice(-6),t.assistKnownCriteria);return t.assistSessionToken=s.sessionToken,s.knownCriteria&&(t.assistKnownCriteria={...t.assistKnownCriteria,...s.knownCriteria}),t.assistHistory.push({role:"assistant",content:s.message}),s}static resetChat(){a.instance&&(a.instance.assistHistory=[],a.instance.assistKnownCriteria={},a.instance.assistSessionToken=void 0)}static reviewStats(e){if(!a.instance)throw new Error("Shimmer not initialized. Call Shimmer.init() first.");return a.instance.client.reviewStats(e)}static destroy(){a.instance?.destroy()}destroy(){this.searchWidget?.destroy(),this.chatWidget?.destroy(),document.getElementById("shimmer-sdk-styles")?.remove(),a.instance=null}};a.instance=null,a.crossSell={async render(e={}){if(!a.instance)throw new Error("Shimmer not initialized. Call Shimmer.init() first.");await new x(a.instance.client,e).render()},async renderInto(e,t,s={}){if(!a.instance)throw new Error("Shimmer not initialized. Call Shimmer.init() first.");await new x(a.instance.client,s).renderInto(e,t)},fetch(e,t=4){if(!a.instance)throw new Error("Shimmer not initialized. Call Shimmer.init() first.");return a.instance.client.crossSell(e,t)},trackProductView(e){if(!a.instance||!Number.isFinite(e)||e<=0)return;let t=R(e);if(t.length===0)return;let s=C(),i=t.map(o=>({product_id:o.ref_id,target_id:o.target_id,role:o.role,event_type:"view_target",session_id:s,position:o.position,metadata:{intent_age_ms:Date.now()-o.ts}}));a.instance.client.crossSellEvents(i).catch(()=>{})},trackPurchase(e){if(!a.instance)return;let t=C(),s=[];for(let i of e){if(!Number.isFinite(i)||i<=0)continue;let o=R(i);for(let r of o)s.push({product_id:r.ref_id,target_id:r.target_id,role:r.role,event_type:"purchase",session_id:t,position:r.position,metadata:{intent_age_ms:Date.now()-r.ts}})}s.length!==0&&a.instance.client.crossSellEvents(s).catch(()=>{})}};var $=a;return q(F);})();
